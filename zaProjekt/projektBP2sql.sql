...
BEGIN
	UPDATE Sezona, Epizoda SET Sezona.Broj_epizoda = Sezona.Broj_epizoda + 1 WHERE Sezona.Sezona_id = Epizoda.Sezona_Sezona_id;
END;$$

USE `projektBP2`$$
CREATE DEFINER = CURRENT_USER TRIGGER `projektBP2`.`Epizoda_BEFORE_DELETE` BEFORE DELETE ON `Epizoda` FOR EACH ROW
BEGIN
	UPDATE Sezona, Epizoda SET Sezona.Broj_epizoda = Sezona.Broj_epizoda - 1 WHERE Sezona.Sezona_id = Epizoda.Sezona_Sezona_id;
END$$
...
USE `projektBP2`$$
CREATE DEFINER = CURRENT_USER TRIGGER `projektBP2`.`Film_BEFORE_UPDATE` BEFORE UPDATE ON `Film` FOR EACH ROW
BEGIN
	IF NEW.Dob >= 18 THEN SET NEW.Za_odrasle = 1;
    ELSE SET NEW.Za_odrasle = 0;
    END IF; 
